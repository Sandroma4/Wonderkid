...
export function LifestyleShopModal({ isOpen, onClose, bankBalance, player, onBuyItem }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 flex justify-center items-center p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-2xl md:rounded-3xl max-w-3xl w-[95%] p-4 md:p-6 shadow-2xl relative text-slate-900 dark:text-white space-y-4 overflow-y-auto max-h-[95vh]">
        <div className="flex justify-between items-center border-b border-slate-300 dark:border-slate-800 pb-3">
          <div>
            <h3 className="text-lg font-black text-slate-900 dark:text-white">🛍️ Boutique & Équipements Pro</h3>
            <p className="text-xs text-slate-600 dark:text-slate-400">Investissez vos gains pour booster vos performances</p>
          </div>
          <button onClick={onClose} className="p-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white font-bold">✕</button>
        </div>
          ...
          {LIFESTYLE_ITEMS.map((item) => {
            ...
            return (
              <div key={item.id} className="p-2 sm:p-3 bg-slate-100 dark:bg-slate-950 rounded-xl sm:rounded-2xl border border-slate-300 dark:border-slate-800 flex flex-col justify-between gap-1 sm:gap-2 group hover:border-emerald-500/50 transition-colors">
                <div>
                  <h4 className="font-bold text-[9px] sm:text-xs text-slate-900 dark:text-white flex flex-col sm:flex-row flex-wrap gap-1 sm:items-center mb-1">
                  ...
                  <button
                    ...
                    className={`w-full sm:w-auto py-1 sm:py-1.5 px-2 sm:px-3 rounded-lg sm:rounded-xl font-black text-[8px] sm:text-[10px] uppercase tracking-wider transition-all flex-shrink-0 ${
                      isOwned
                        ...
                        : canAfford
                        ? 'bg-emerald-500 hover:bg-emerald-400 text-slate-900 dark:text-slate-100 shadow-md active:scale-95'
