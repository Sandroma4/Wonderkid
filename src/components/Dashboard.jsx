...
const AnimatedStatBar = ({ label, oldVal, newVal, gain }) => {
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full max-w-[160px] bg-slate-50/80 dark:bg-slate-900/80 p-2.5 rounded-xl border border-slate-300/80 dark:border-slate-700/50 shadow-inner">
...
export function Dashboard({
  ...
  return (
    <>
      ...
      {isRetired ? (
        <div className="app-typography min-h-[100dvh] bg-slate-200 dark:bg-[#0F172A] p-4 md:p-8 text-slate-800 dark:text-slate-100 relative overflow-y-auto flex flex-col items-center justify-center font-sans">
          ...
          {(() => {
            ...
            return (
              <div className="max-w-3xl w-full bg-slate-50/95 dark:bg-slate-900/95 border border-slate-300/80 dark:border-slate-700/80 backdrop-blur-md rounded-3xl p-6 md:p-8 shadow-2xl z-10 flex flex-col items-center text-center my-auto">
                <div className="mb-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 w-full mt-2">
                  <div className="bg-slate-100/90 dark:bg-slate-800/80 border border-slate-700/60 p-3.5 rounded-2xl text-center shadow-inner">
                  ...
                  {groupedPalmares.length > 0 ? (
                    <div className="flex flex-wrap gap-2 justify-center">
                      {groupedPalmares.slice(0, 12).map((trophy, idx) => (
                        <span key={idx} className="bg-slate-50/90 dark:bg-slate-900/90 border border-amber-500/30 text-amber-600 dark:text-amber-300 px-3 py-1.5 rounded-xl text-xs font-semibold shadow-sm flex items-center gap-1.5">
                ...
                {player.careerHistory && player.careerHistory.length > 0 && (
                  <details className="w-full bg-slate-800/70 rounded-2xl border border-slate-700/60 mt-4 group">
                    <summary className="p-4 heading-typography font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider text-xs cursor-pointer list-none flex justify-between items-center hover:text-white transition-colors">
                    ...
                    <div className="overflow-x-auto px-4 pb-4 pt-0">
                      <table className="w-full text-xs text-left">
                        <thead className="text-[10px] text-slate-500 dark:text-slate-500 dark:text-slate-400 uppercase bg-slate-50/80 dark:bg-slate-900/80 rounded-lg">
                          <tr>
                        ...
                        <tbody>
                          {player.careerHistory.map((season, i) => (
                            <tr key={i} className="border-b border-slate-700/40 last:border-0 hover:bg-slate-700/30 transition-colors">
                              <td className="px-3 py-2 font-medium text-slate-700 dark:text-slate-200">{season.year} <span className="text-[10px] text-slate-500 dark:text-slate-500 dark:text-slate-400">({season.age} ans)</span></td>
                              <td className="px-3 py-2">
                                <div className="flex items-center gap-2">
                                  <span className="font-semibold text-slate-800 dark:text-white">{season.club}</span>
                ...
                {multiplayerContext && opponent && (
                  <div className={`w-full bg-slate-50/80 dark:bg-slate-900/80 p-5 rounded-2xl border mt-4 shadow-inner text-center ${multiplayerContext.isCoopMode ? 'border-emerald-500/50' : 'border-cyan-500/50'}`}>
                    ...
                    {!opponent.isRetired ? (
                      <p className="text-slate-500 dark:text-slate-500 dark:text-slate-400 text-sm animate-pulse">En attente de la fin de carrière de {opponent.name}...</p>
                    ) : (
                      <div className="flex flex-col items-center gap-4">
                        <div className="flex justify-between w-full items-center px-4">
                          <div className="text-center">
                            <p className="text-[10px] text-slate-500 dark:text-slate-500 dark:text-slate-400 uppercase font-bold">{player.name}</p>
                          ...
                          <div className="text-center">
                            <p className="text-[10px] text-slate-500 dark:text-slate-500 dark:text-slate-400 uppercase font-bold">{opponent.name}</p>
                          ...
                          if (multiplayerContext.isCoopMode) {
                            ...
                            return (
                              <div className="mt-4 border-t border-slate-300/80 dark:border-slate-700/50 pt-3 w-full">
                                <p className="text-[10px] text-slate-500 dark:text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">Score Total Coopératif</p>
                            ...
                            return <div className="text-slate-600 dark:text-slate-300 font-black text-xl md:text-2xl uppercase tracking-widest mt-2">Égalité 🤝</div>;
                          }
                        })()}
                      </div>
                    )}
          ...
          <div className="max-w-4xl w-full z-10 flex flex-col justify-center py-8">
            ...
            <div className="flex flex-col md:grid md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-5 min-h-0 shrink">
              {(multiplayerContext?.isCoopMode && !multiplayerContext?.isHost) ? (
                <div className="col-span-full w-full flex flex-col items-center justify-center p-8 bg-slate-100/90 dark:bg-slate-800/80 rounded-2xl border border-slate-300 dark:border-slate-700">
                ...
                return (
                  <div 
                    key={offer.id} 
                    className="bg-slate-100 dark:bg-slate-800 border-2 rounded-xl md:rounded-2xl p-2 md:p-6 flex flex-row md:flex-col justify-between items-center md:items-stretch shadow-2xl transition-all duration-300 hover:scale-[1.02] flex-1 min-h-0 gap-2 md:gap-0"
        ...
        <div className="app-typography h-[100dvh] text-slate-800 dark:text-slate-100 p-2 md:p-6 flex flex-col items-center justify-center relative overflow-hidden" style={clubBackgroundStyle}>
          ...
          <div className="absolute inset-0 bg-football-pattern pointer-events-none opacity-10"></div>
          <div className="max-w-3xl w-full bg-slate-50/95 dark:bg-slate-900/95 border border-slate-300/80 dark:border-slate-700/50 backdrop-blur-md rounded-2xl md:rounded-3xl p-3 md:p-7 shadow-2xl z-10 flex flex-col md:flex-row gap-2 md:gap-6 items-center justify-center h-[95%] md:h-auto overflow-hidden">
            ...
            <div className="flex-1 space-y-2 md:space-y-5 w-full flex flex-col min-h-0 shrink -mt-16 md:mt-0">
              <div className="shrink-0 text-center md:text-left">
                <span className="heading-typography text-[11px] font-semibold px-3 py-1 rounded-full uppercase tracking-wider shadow-sm bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 md:gap-3">
                <div className="bg-slate-100/90 dark:bg-slate-800/80 p-1.5 md:p-3 rounded-2xl border border-slate-300/80 dark:border-slate-700/50 text-center shadow-inner">
              ...
              {player.valueHistory && player.valueHistory.length > 1 && (
                <div className="bg-slate-100/90 dark:bg-slate-800/80 p-3 md:p-4 rounded-2xl border border-slate-300/80 dark:border-slate-700/50 shadow-inner w-full mt-2 hidden md:block">
              ...
              {seasonStats.financials && (
                <div className="bg-slate-100/90 dark:bg-slate-800/80 p-3 md:p-4 rounded-2xl border border-slate-300/80 dark:border-slate-700/50 shadow-inner w-full mt-2 text-xs">
              ...
              <div className="bg-slate-100/90 dark:bg-slate-800/80 p-2 md:p-3.5 rounded-2xl border border-slate-300/80 dark:border-slate-700/50 space-y-0.5 md:space-y-1 shadow-inner text-xs overflow-y-auto">
              ...
              {seasonStats.tournaments && (
                <div className="bg-slate-100/90 dark:bg-slate-800/80 p-2 md:p-3.5 rounded-2xl border border-slate-300/80 dark:border-slate-700/50 shadow-inner text-xs space-y-1 md:space-y-1.5 hidden md:block">
              ...
              {seasonStats.statGains && Object.keys(seasonStats.statGains).length > 0 && (
                <div className="bg-slate-100/90 dark:bg-slate-800/80 p-2 md:p-4 rounded-2xl border border-slate-300/80 dark:border-slate-700/50 shadow-inner text-xs space-y-1.5 md:space-y-3 mt-1.5 md:mt-4">
        ...
        <div className="app-typography min-h-[100dvh] flex flex-col items-center justify-center relative overflow-hidden p-4" style={clubBackgroundStyle}>
          <div className="absolute inset-0 bg-football-pattern pointer-events-none opacity-10"></div>
          <div className="max-w-3xl w-full bg-slate-50/90 dark:bg-slate-900/90 border-2 border-rose-500/50 rounded-3xl p-4 md:p-8 shadow-2xl z-10 text-slate-800 dark:text-white relative">
            {gameState.interactiveMatchFinalOutcome ? (
               <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 p-6 md:p-8 rounded-2xl text-center shadow-inner space-y-6">
            ...
            <div className="text-center mb-8">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="bg-slate-100 dark:bg-slate-800 text-amber-600 dark:text-amber-400 border border-amber-500/50 text-sm font-bold px-4 py-1 rounded-full tracking-widest">
            ...
            {interactiveMatchResult ? (
               <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/80 p-6 md:p-8 rounded-2xl text-center space-y-6 shadow-2xl backdrop-blur-md">
        ...
        <div className="app-typography min-h-[100dvh] flex flex-col items-center justify-center relative overflow-hidden" style={clubBackgroundStyle}>
          ...
          <div className="app-typography min-h-[100dvh] text-slate-800 dark:text-slate-100 p-2 md:p-6 flex flex-col items-center justify-center relative overflow-y-auto bg-slate-300 dark:bg-slate-950">
            <div className="absolute inset-0 bg-football-pattern pointer-events-none opacity-10"></div>
          ...
          <div className="max-w-4xl w-full flex flex-col z-10 justify-center py-8">
            ...
            <div className="flex flex-col md:grid md:grid-cols-2 gap-2 md:gap-5 min-h-0 shrink">
              {transferMarketOffers.map((offer) => {
                ...
                return (
                  <div 
                    key={offer.id} 
                    className="bg-slate-100 dark:bg-slate-800 rounded-xl md:rounded-3xl p-2 md:p-6 flex flex-row md:flex-col justify-between items-center md:items-stretch shadow-xl border-2 md:border-4 transition-all duration-300 hover:scale-[1.02] flex-1 min-h-0 gap-2 md:gap-0"
                    style={{ borderColor: offerTheme.accent }}
                  >
                    <div className="min-h-0 overflow-hidden flex-1 flex flex-col justify-center">
                      <div className="flex items-center gap-1 md:gap-2 mb-0.5 md:mb-3">
            ...
            <div className="mt-4 md:mt-8 flex justify-center shrink-0">
              <button 
                ...
                className="heading-typography py-3 px-6 md:py-4 md:px-8 rounded-xl font-bold text-xs md:text-sm uppercase tracking-wider text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-700 hover:bg-slate-300 dark:hover:bg-slate-700 hover:text-white transition-all shadow-lg"
              >
                🏠 Rester à {club.name}
              </button>
            </div>
        ...
        >
          ...
          <div className="max-w-5xl mx-auto space-y-1.5 md:space-y-4 relative z-10">
            <div className="flex justify-center items-center gap-2 md:gap-4 mb-1.5 md:mb-4">
              <div className="bg-slate-50/90 dark:bg-slate-900/90 backdrop-blur-sm border border-slate-300/80 dark:border-slate-700/50 px-3 py-1.5 md:px-6 md:py-2 rounded-xl md:rounded-2xl shadow-2xl flex items-center justify-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
              ...
              <div className={`${activeMobileTab !== 'joueur' ? 'hidden md:flex' : 'flex'} flex-col space-y-4 lg:col-span-1`}>
                ...
                <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-3xl p-0 md:p-4 shadow-2xl flex flex-col items-center h-fit">
                  <button onClick={() => window.print()} className="w-full mb-2 text-xs font-bold uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-white py-1.5 rounded-xl shadow-md hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">📸 Exporter ma Carte</button>
                  <PlayerCard player={player} club={club} cardType="auto" />
                </div>
                <div className="hidden md:block">
                  {/* STATS DÉTAILLÉES (Toujours visible sur desktop) */}
                  <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-2xl p-3 shadow-2xl">
                    ...
                    <div className="border-t border-slate-300/80 dark:border-slate-700/50 pt-3 grid grid-cols-2 gap-1.5">
                      {['pace', 'dribbling', 'finishing', 'defense', 'passing', 'physical'].map((attr) => {
                        ...
                        const diff = eff - val;
                        return (
                          <div key={attr} className="bg-slate-100 dark:bg-slate-800 px-2 py-1.5 rounded-md border border-slate-300/80 dark:border-slate-700/50 flex justify-between items-center shadow-sm">
                ...
                <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-2xl p-3 shadow-2xl">
                  <button onClick={() => setIsTrophiesOpen(!isTrophiesOpen)} className="w-full heading-typography text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider flex justify-between items-center bg-slate-100 dark:bg-slate-800 p-2 rounded-xl">
...
{/* GRAPH VALEUR MARCHANDE */}
                {player.valueHistory && player.valueHistory.length > 0 && (
                  <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-2xl p-4 md:p-5 shadow-2xl flex flex-col h-fit relative">
              ...
              <div className={`${activeMobileTab !== 'terrain' ? 'hidden md:flex' : 'flex'} lg:col-span-2 flex-col space-y-4`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {/* Club Actuel - DESKTOP */}
                  <div className="hidden md:flex bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-2xl p-4 shadow-2xl flex-col justify-between gap-4 h-full relative overflow-hidden">
                  ...
                  {/* Club Actuel - MOBILE */}
                  <div className="flex md:hidden bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-2xl p-4 shadow-2xl items-center justify-between gap-2 h-full relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-3 opacity-20 transform translate-x-2 -translate-y-2">
                      <div className="w-16 h-16 rounded-full blur-xl" style={{ background: `linear-gradient(135deg, ${club.primary} 0%, ${club.secondary} 100%)` }} />
                    </div>
                  ...
                  {/* Gauges (États) */}
                  <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-2xl p-3 shadow-2xl flex flex-col justify-center h-full">
                    <div className="space-y-2">
                      <div>
                        ...
                        <div className="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden border border-slate-300/80 dark:border-slate-700/50">
                          <div className={`${player.form < 30 ? 'bg-rose-500' : 'bg-amber-500'} h-full rounded-full transition-all shadow-sm`} style={{ width: `${player.form}%` }} />
                        </div>
                      </div>
                      <div>
                ...
                >
                  ...
                  <span className="bg-slate-50/50 dark:bg-slate-900/50 px-2 py-1 rounded-md text-[10px] font-black ml-2 shadow-inner">
                    {bankBalance.toLocaleString('fr-FR')} €
                  </span>
                </button>

                <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-2xl p-4 md:p-5 shadow-2xl flex flex-col h-fit">
                  <div>
                  ...
                  {activeOutcome ? (
                    ...
                    <div className="space-y-1.5 my-2 md:my-3 sticky bottom-14 md:static z-20 bg-slate-50 dark:bg-slate-900 md:bg-transparent p-2 md:p-0 -mx-2 md:mx-0 rounded-xl shadow-[0_-5px_15px_-5px_rgba(0,0,0,0.5)] md:shadow-none">
                      {currentEvent?.options?.map((option, idx) => (
                        <button 
                          key={idx} 
                          onClick={() => { playSound('click'); onSelectOption(idx); }} 
                          className="w-full text-left p-3 md:p-4 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:border-emerald-500 hover:bg-slate-300 dark:hover:bg-slate-700 transition-all duration-150 flex items-center justify-between group shadow-sm"
                        >
                          <div className="flex items-center gap-2 md:gap-3 pr-2">
                            <span className="heading-typography text-[8px] md:text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md bg-slate-300 dark:bg-slate-950 text-emerald-600 dark:text-emerald-400 shadow-sm flex-shrink-0">{option.typeTag}</span>
...
{/* STATS DÉTAILLÉES (Toujours visible) */}
                <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-2xl p-3 shadow-2xl">
                  <button onClick={() => setIsMobileStatsOpen(!isMobileStatsOpen)} className="w-full heading-typography text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider flex justify-between items-center mb-1 bg-slate-100 dark:bg-slate-800 p-2 rounded-xl">
                    <span className="flex items-center gap-1.5">📊 Stats Détaillées</span>
              ...
              <div className={`${activeMobileTab !== 'carriere' ? 'hidden md:flex' : 'flex'} flex-col space-y-4 lg:col-span-1`}>
                ...
                {rival && (() => {
                  ...
                  return (
                    <div className="bg-slate-50 dark:bg-slate-900 border border-rose-900/50 rounded-3xl p-0 md:p-4 shadow-2xl flex flex-col items-center h-fit relative overflow-hidden">
                ...
                {rival && (() => {
                  ...
                  return (
                    <div className="bg-slate-50 dark:bg-slate-900 border border-rose-900/50 rounded-3xl p-4 shadow-2xl flex flex-col relative overflow-hidden gap-4">
                      <div className="absolute -top-4 -right-4 p-4 opacity-10 text-6xl transform rotate-12 pointer-events-none">⚔️</div>
                      ...
                      <div className="flex items-center gap-3 relative z-10 bg-slate-100/50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-300/80 dark:border-slate-700/50">
                      ...
                      <div className="w-full flex flex-row items-center justify-around bg-slate-100/50 dark:bg-slate-800/50 p-2 rounded-xl border border-slate-300 dark:border-slate-700 relative z-10 gap-1 md:gap-0">
                ...
                {player.traits && player.traits.length > 0 && (
                  <div className="bg-slate-50 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-3xl p-4 shadow-2xl">
                    ...
                    <div className="flex flex-col gap-2">
                      {player.traits.map(traitId => {
                        ...
                        return (
                          <div key={traitId} className="flex items-center gap-3 bg-slate-100/90 dark:bg-slate-800/80 p-2 rounded-xl border border-slate-300 dark:border-slate-700">
          ...
          {/* MOBILE BOTTOM NAV */}
          <div className="fixed bottom-0 left-0 w-full bg-slate-50 dark:bg-slate-900 border-t border-slate-300 dark:border-slate-800 md:hidden z-50 flex justify-around p-2 pb-safe shadow-2xl">
             <button 
