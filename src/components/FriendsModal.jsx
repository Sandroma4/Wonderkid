    ...
    <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-900 border border-emerald-500/50 rounded-2xl md:rounded-3xl p-4 md:p-6 w-full max-w-md shadow-2xl relative flex flex-col max-h-[85vh]">
        <button 
          ...
          className="absolute top-4 right-4 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-slate-900 dark:text-white transition-colors text-xl font-black"
        >
          X
        </button>
        ...
        <div className="flex bg-white dark:bg-slate-800 rounded-lg p-1 mb-4 flex-shrink-0">
          ...
          >
            Requêtes
            {requests.length > 0 && (
              <span className="absolute -top-1 -right-1 bg-rose-500 text-slate-900 dark:text-slate-900 dark:text-white text-[9px] w-4 h-4 flex items-center justify-center rounded-full">{requests.length}</span>
            )}
          </button>
          <button 
                  ...
                  <div key={req.id} className="bg-white/80 dark:bg-slate-800/80 p-3 rounded-xl border border-slate-300 dark:border-slate-700 flex flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                      <button 
                        ...
                        className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-slate-900 dark:text-slate-900 dark:text-white py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors"
                      >
                        Accepter
                      </button>
                      <button 
                ...
                {searchResults.map(res => (
                  <div key={res.id} className="bg-white/80 dark:bg-slate-800/80 p-3 rounded-xl border border-slate-300 dark:border-slate-700 flex justify-between items-center">
                    ...
                    <button 
                      ...
                      className="bg-emerald-600 hover:bg-emerald-500 text-slate-900 dark:text-slate-900 dark:text-white px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors"
                    >
                      Ajouter
                    </button>
                  </div>
