...
export const CareerHistory = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0F172A] p-6 text-slate-800 dark:text-slate-200 relative overflow-hidden font-sans flex flex-col items-center">
      ...
      <div className="w-full max-w-3xl relative z-10">
        <div className="flex items-center justify-between mb-8">
          <button 
            ...
            className="text-slate-900 dark:text-slate-900 dark:text-white bg-white/80 dark:bg-slate-800/80 hover:bg-slate-100 dark:bg-slate-700 p-3 rounded-xl transition-all active:scale-95 border border-slate-300 dark:border-slate-700 whitespace-nowrap"
          >
            ← Retour
          </button>
        ...
        <div className="bg-white/80 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-700 rounded-3xl p-6 shadow-2xl backdrop-blur-sm min-h-[60vh]">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-slate-600 dark:text-slate-400 font-semibold text-lg">Aucune carrière enregistrée pour le moment.</p>
            ...
            <div className="space-y-3">
              {scores.map((entry, index) => {
                ...
                let bgClass = "bg-slate-100 dark:bg-slate-700/50 hover:bg-slate-100 dark:bg-slate-700";
                let rankColor = "text-slate-500 dark:text-slate-500";
                
                if (index === 0) { bgClass = "bg-amber-900/40 border border-amber-500/30"; rankColor = "text-amber-400"; }
                else if (index === 1) { bgClass = "bg-slate-400/20 border border-slate-300/30"; rankColor = "text-slate-700 dark:text-slate-300"; }
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-bold text-slate-900 dark:text-slate-900 dark:text-white text-lg">{entry.pseudo}</p>
                          {entry.nationality && (
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-400 font-medium uppercase tracking-wider flex items-center gap-2">
                          <span>{entry.player_name || 'Inconnu'}</span>
                          <span>•</span>
                          <span>OVR Max : <span className="text-emerald-400 font-bold">{entry.ovr}</span></span>
                          {entry.major_trophies > 0 && (
                            <>
                              <span>•</span>
                              <span className="text-amber-400 flex items-center gap-1">🏆 {entry.major_trophies}</span>
                            </>
                          )}
                        </p>
                      </div>
