...
export const CareerHistory = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] p-6 text-slate-200 relative overflow-hidden font-sans flex flex-col items-center">
      ...
      <div className="w-full max-w-3xl relative z-10">
        ...
        <div className="bg-slate-800/80 border border-slate-700 rounded-3xl p-6 shadow-2xl backdrop-blur-sm min-h-[60vh]">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-slate-400 font-semibold text-lg">Aucune carrière enregistrée pour le moment.</p>
              <p className="text-slate-500 text-sm mt-2">Termine une carrière en étant connecté pour inaugurer ton historique !</p>
            </div>
          ) : (
            <div className="space-y-3">
              {scores.map((entry, index) => {
                ...
                else if (index === 1) { bgClass = "bg-slate-400/20 border border-slate-300/30"; rankColor = "text-slate-300"; }
                ...
                return (
                  <div key={entry.id} className={`${bgClass} rounded-2xl p-4 flex items-center justify-between transition-colors`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-bold text-white text-lg">{entry.pseudo}</p>
                          {entry.nationality && (
                            <span className="text-[10px] bg-slate-800 px-1.5 py-0.5 rounded border border-slate-700">{entry.nationality}</span>
                          )}
                        </div>
                    ...
                    <div className="text-right">
                      <p className="text-xs text-slate-500 uppercase font-bold tracking-widest mb-0.5">Score</p>
