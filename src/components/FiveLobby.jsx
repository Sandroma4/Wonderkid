...
export const FiveLobby = ({ onStart, onBack, multiplayerContext }) => {
  ...
  const [playerName, setPlayerName] = useState(localStorage.getItem('golden_xi_pseudonym') || t('five.unknown_player', 'Joueur Inconnu'));
        ...
        <div className="relative flex flex-col items-center justify-center mb-4 md:mb-8 w-full max-w-sm mx-auto">
          <div className="w-full text-center px-4">
            <h2 className="heading-typography text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500 uppercase tracking-wider mb-2 leading-none">
              {t('five.title', 'FIVE (5v5)')}
            </h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              {t('five.desc', 'Affrontez d\'autres joueurs avec votre équipe composée de vos 5 meilleures cartes du Hall of Fame.')}
            </p>
          </div>
        </div>
          ...
          <div className="w-full max-w-sm mx-auto space-y-4">
            {myTeams.length === 0 ? (
              <div className="p-4 bg-orange-500/10 border border-orange-500/50 rounded-xl text-orange-400 font-bold mb-4">
                {t('five.need_team_warning', 'Vous devez d\'abord créer une équipe Five dans le menu "Gestion Five" avant de pouvoir jouer en ligne.')}
              </div>
            ) : (
              <>
                <button 
                ...
                >
                  {t('five.create_room', 'Créer un Salon')}
                </button>

                <div className="flex items-center gap-2">
                  <div className="h-px bg-slate-300 dark:bg-slate-700 flex-1"></div>
                  <span className="text-slate-500 dark:text-slate-500 text-xs uppercase font-bold">{t('five.or', 'ou')}</span>
                  <div className="h-px bg-slate-300 dark:bg-slate-700 flex-1"></div>
                </div>
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    placeholder={t('five.join_placeholder', 'Code à 4 lettres')}
                    maxLength={4}
          ...
          <div className="w-full flex flex-col items-center">
            {isHost && (
              <div className="bg-white/90 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-700 rounded-xl p-4 mb-6 w-full max-w-sm shadow-inner">
                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">{t('five.room_code', 'Code du Salon')}</p>
                <p className="text-4xl font-mono font-black text-orange-500 tracking-widest">{roomId}</p>
                <p className="text-xs text-slate-500 mt-2">{t('five.share_code', 'Partagez ce code avec votre adversaire')}</p>
              </div>
            )}

            <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 text-left">
              <div className="bg-slate-800 rounded-2xl p-4 border border-slate-700">
                <h3 className="text-orange-400 font-bold uppercase text-xs tracking-wider mb-4 border-b border-slate-700 pb-2">{t('five.players_in_room', 'Joueurs dans le salon')}</h3>
                {players.map(p => (
                  <div key={p.playerId} className="flex items-center justify-between bg-slate-900 p-3 rounded-lg border border-slate-700 mb-2">
                    ...
                    {p.ready ? (
                      <span className="text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded font-bold uppercase">{t('five.ready', 'Prêt')}</span>
                    ) : (
                      <span className="text-[10px] bg-slate-500/20 text-slate-400 px-2 py-1 rounded font-bold uppercase">{t('five.choosing_team', 'Choix équipe...')}</span>
                    )}
                  </div>
                ))}
                {players.length < 2 && (
                  <div className="flex items-center justify-center bg-slate-900/50 p-3 rounded-lg border border-slate-700 border-dashed animate-pulse mt-2">
                    <span className="text-slate-500 italic text-sm">{t('five.waiting_opponent', 'En attente d\'un adversaire...')}</span>
                  </div>
                )}
              </div>

              <div className="bg-slate-800 rounded-2xl p-4 border border-slate-700 flex flex-col h-full">
                <h3 className="text-orange-400 font-bold uppercase text-xs tracking-wider mb-4 border-b border-slate-700 pb-2">{t('five.your_team_header', 'Votre Équipe')}</h3>
            ...
            {isHost ? (
              ...
              >
                {players.length < 2 ? t('five.waiting_players', 'Attente des joueurs') : (!allPlayersReady ? t('five.waiting_teams', 'En attente des équipes') : t('five.start_match', 'Lancer le Match Five'))}
              </button>
            ) : (
              <div className="w-full max-w-sm py-4 bg-slate-800 text-slate-400 rounded-xl font-bold uppercase tracking-wide border border-slate-700 flex justify-center items-center gap-2">
                {!selectedTeamId ? t('five.select_team_hint', 'Sélectionnez une équipe ↑') : (
                  <>
                    ...
                    {t('five.waiting_host', 'Attente de l\'hôte...')}
                  </>
                )}
              </div>
            )}
