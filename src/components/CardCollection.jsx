...
export const CardCollection = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-3 md:p-8 relative overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto">
        {collection.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-8 bg-white dark:bg-slate-900/50 rounded-3xl border border-slate-200 dark:border-slate-800 backdrop-blur-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-8 auto-rows-max">
            {collection.map((card, idx) => (
              <div key={card.id || idx} className="flex flex-col items-center bg-white dark:bg-slate-900/60 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 backdrop-blur-sm hover:border-emerald-500/50 transition-colors shadow-xl group">
                <div className="mb-4 transform group-hover:scale-105 transition-transform duration-300">
                  <PlayerCard player={card} club={card.club} />
                </div>
                
                <div className="w-full mt-2 bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl border border-slate-200 dark:border-slate-800/80 shadow-inner space-y-2">
