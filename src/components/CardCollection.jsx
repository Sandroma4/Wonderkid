...
export const CardCollection = ({ onBack }) => {
  ...
  const handleApplyCosmetic = (cosmeticId) => {
    ...
    if (cosmeticId === 'default') {
      ...
      if (!inventory[cosmeticId] || inventory[cosmeticId] <= 0) {
        alert(t('card_collection.cosmetic_not_owned', "Vous ne possédez plus cet objet !"));
        return;
      }
  ...
  return (
    <div className="h-[100dvh] bg-emerald-200 dark:bg-slate-950 p-3 md:p-8 relative overflow-y-auto flex flex-col">
      <div className="flex items-center justify-between mb-6 md:mb-8 w-full max-w-6xl mx-auto pt-2 md:pt-4">
        ...
        >
          <span className="text-xl leading-none">←</span> <span className="hidden md:inline font-bold ml-2 uppercase tracking-wider text-[10px] md:text-xs">{t('card_collection.back', 'Retour')}</span>
        </button>
        <div className="flex-1 text-center px-2">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 text-[10px] md:text-base font-medium">{t('card_collection.subtitle', 'Votre collection de légendes. L\'histoire s\'écrit ici.')}</p>
        </div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto">
        {collection.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-8 bg-white/50 dark:bg-slate-900/50 rounded-3xl border border-slate-300 dark:border-slate-800 backdrop-blur-sm">
            ...
            <h2 className="heading-typography text-2xl font-bold text-slate-800 dark:text-white mb-2">{t('card_collection.empty_title', 'Album vide')}</h2>
            <p className="text-slate-500 dark:text-slate-500 dark:text-slate-400">{t('card_collection.empty_desc', 'Terminez une carrière pour voir votre carte apparaître ici !')}</p>
          </div>
        ) : (
              ...
              <div key={card.id || idx} className="flex flex-col items-center bg-slate-900/60 p-5 rounded-3xl border border-slate-300 dark:border-slate-800 backdrop-blur-sm hover:border-emerald-500/50 transition-colors shadow-xl group relative">
                ...
                {!isGk(card.position) && (
                  <button 
                    ...
                    title={t('card_collection.customize_card', 'Personnaliser la carte')}
                  >
                    ✨
                  </button>
                )}
                ...
                <div className="w-full mt-2 bg-emerald-200 dark:bg-slate-950 p-3 rounded-2xl border border-slate-800/80 shadow-inner space-y-2">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-500 dark:text-slate-500 font-medium">{t('card_collection.end_of_career', 'Fin de carrière')}</span>
                    <span className="text-slate-600 dark:text-slate-300 font-bold">{card.date}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-500 dark:text-slate-500 font-medium">{t('card_collection.ballon_dor', 'Ballons d\'Or')}</span>
                    <span className="text-amber-600 dark:text-amber-400 font-bold flex items-center gap-1">
                      {card.ballonDor > 0 ? Array(Math.min(card.ballonDor, 5)).fill('⭐').join('') : t('card_collection.none', 'Aucun')}
                      {card.ballonDor > 5 && ` (+${card.ballonDor - 5})`}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-500 dark:text-slate-500 font-medium">{t('card_collection.major_trophies', 'Trophées Majeurs')}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-[#0F172A] w-full max-w-2xl rounded-3xl p-6 border border-slate-700 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center mb-6 shrink-0">
              <h2 className="text-2xl font-black text-white">{t('card_collection.customize_name', 'Personnaliser {{name}}', { name: selectedCardForCosmetic.lastName })} ({selectedCardForCosmetic.ovr} OVR)</h2>
            ...
            <div className="overflow-y-auto flex-1 pr-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 flex justify-between items-center cursor-pointer hover:border-slate-500" onClick={() => handleApplyCosmetic('default')}>
                  <span className="text-white font-bold">{t('card_collection.remove_cosmetic', 'Retirer (Classique)')}</span>
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white font-bold">{t(`cosmetics.${cosmetic.id}`, cosmetic.name)}</span>
                        <span className="text-xs bg-slate-700 px-2 py-1 rounded text-slate-300">{t('card_collection.owned', 'Possédé:')} {ownedQty}</span>
                      </div>
                      <div className="text-xs text-slate-400">{t('card_collection.restricted', 'Restreint:')} {cosmetic.category}</div>
                    </div>
                  );
                })}
              </div>
              ...
              {getAvailableCosmeticsForCard(selectedCardForCosmetic.ovr).length === 0 && (
                <p className="text-rose-400 text-sm mt-4 text-center">{t('card_collection.no_cosmetics', 'Aucun design cosmétique ne correspond à la note ({{ovr}} OVR) de cette carte.', { ovr: selectedCardForCosmetic.ovr })}</p>
              )}
            </div>
          </div>
        </div>
