  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0F172A] p-4 md:p-8 font-sans flex flex-col relative overflow-hidden">
      ...
      <div className="max-w-4xl w-full mx-auto flex-1 flex flex-col relative z-10">
        ...
        {/* Progress Bar */}
        <div className="w-full bg-white dark:bg-slate-800 rounded-full h-2.5 mb-8 border border-slate-300 dark:border-slate-700">
          ...
          {['all', 'common', 'rare', 'epic', 'legendary'].map(tab => (
            <button 
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-all border
                ${activeTab === tab 
                  ? 'bg-slate-100 dark:bg-slate-700 text-white border-slate-500 shadow-md scale-105' 
                  : 'bg-slate-800/50 text-slate-600 dark:text-slate-400 border-slate-700/50 hover:bg-white dark:bg-slate-800 hover:text-slate-700 dark:text-slate-300'}`}
            >
              {tab === 'all' ? 'Tous' : tab}
            </button>
          ))}
              ...
              return (
                <div 
                  ...
                  className={`relative overflow-hidden rounded-2xl p-4 flex items-center gap-4 transition-all duration-300 border 
                    ${isUnlocked ? 'bg-white/80 dark:bg-slate-800/80 ' + rarityBorders[ach.rarity] : 'bg-white dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 opacity-60 grayscale'}`
                  }
                >
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center text-3xl shrink-0 
                    ${isUnlocked ? 'bg-gradient-to-br ' + rarityColors[ach.rarity] : 'bg-white dark:bg-slate-800'}`}
                  >
                    {isUnlocked ? ach.icon : '🔒'}
                  </div>
                  <div>
                    <h3 className={`font-bold text-lg flex items-center gap-2 ${isUnlocked ? 'text-slate-900 dark:text-slate-900 dark:text-white' : 'text-slate-400'}`}>
                      {ach.title}
