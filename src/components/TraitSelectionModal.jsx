    ...
    <div className="fixed inset-0 bg-emerald-200/90 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/50 rounded-2xl md:rounded-3xl p-4 md:p-6 max-w-2xl w-[95%] shadow-2xl relative overflow-y-auto max-h-[90vh]">
        ...
        <div className="text-center mb-8 mt-2">
          ...
          <h2 className="heading-typography text-2xl font-black text-slate-800 dark:text-white uppercase tracking-wider mb-2">{t('trait_selection.title', '18 Ans : L\'Âge de Raison')}</h2>
          <p className="text-slate-500 dark:text-slate-500 dark:text-slate-400">{t('trait_selection.desc_1', 'Vous passez un cap important dans votre développement. Il est temps de définir votre style de jeu principal. Choisissez votre')} <span className="text-amber-600 dark:text-amber-400 font-bold">{t('trait_selection.signature_trait', 'Trait Signature')}</span>.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {traits.map((trait) => (
            ...
            >
              ...
              <h3 className="heading-typography font-bold text-slate-800 dark:text-white text-lg mb-2 text-center">{t(`traits.${trait.id}.name`, trait.name)}</h3>
              <p className="text-xs text-slate-500 dark:text-slate-500 dark:text-slate-400 text-center leading-relaxed">{t(`traits.${trait.id}.desc`, trait.description)}</p>
            </button>
          ))}
        </div>
      </div>
