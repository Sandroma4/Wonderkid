        ...
        <div className="flex flex-col md:flex-row items-start justify-between gap-4 md:gap-0 mt-4 md:mt-8 mb-4 md:mb-8 p-1 md:p-2 w-full">
          {palmares.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center opacity-50">
              ...
              <p className="text-slate-900 dark:text-white text-lg font-medium">Aucun trophée remporté pour le moment.</p>
            ...
            <div className="flex-1 overflow-y-auto pr-1 md:pr-4 pb-8 custom-scrollbar grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 content-start w-full">
              {Object.values(palmares.reduce((acc, t) => {
                ...
                <div key={idx} className="bg-slate-800/60 border border-amber-500/30 p-4 rounded-2xl flex flex-col items-center justify-center text-center hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors shadow-lg relative">
                  ...
                  <span className="absolute -top-2 -right-2 bg-gradient-to-r from-amber-500 to-amber-600 text-slate-900 dark:text-white font-black text-xs md:text-sm px-2.5 py-1 rounded-full shadow-[0_0_10px_rgba(245,158,11,0.5)] border border-amber-300/50 z-10">
                    x{trophy.count}
                  </span>
                </div>
              ))}
