...
def revert(content):
    # This reverts the previously mapped classes back to the original dark-only classes
...
def apply_new(content):
    # New softer light mode mapping
    # -----------------------------
    # Instead of bg-white (too bright), we use bg-slate-50 for containers.
    # Instead of bg-slate-50 for background, we use bg-slate-200 for main background.
    # Text contrast softened from slate-900 to slate-800.
    
    mapping = {
        # App Backgrounds (softer, slate-200 instead of slate-50)
        r'(?<![\w:-])bg-\[\#0F172A\](?![\w/-])': 'bg-slate-200 dark:bg-[#0F172A]',
        r'(?<![\w:-])bg-slate-950(?![\w/-])': 'bg-slate-300 dark:bg-slate-950',
        
        # Containers (softer, slate-50/100 instead of pure white)
        r'(?<![\w:-])bg-slate-900(?![\w/-])': 'bg-slate-50 dark:bg-slate-900',
        r'(?<![\w:-])bg-slate-900/95(?![\w/-])': 'bg-slate-50/95 dark:bg-slate-900/95',
        r'(?<![\w:-])bg-slate-900/90(?![\w/-])': 'bg-slate-50/90 dark:bg-slate-900/90',
        r'(?<![\w:-])bg-slate-900/80(?![\w/-])': 'bg-slate-50/80 dark:bg-slate-900/80',
        r'(?<![\w:-])bg-slate-900/50(?![\w/-])': 'bg-slate-50/50 dark:bg-slate-900/50',
        
        r'(?<![\w:-])bg-slate-800(?![\w/-])': 'bg-slate-100 dark:bg-slate-800',
        r'(?<![\w:-])bg-slate-800/90(?![\w/-])': 'bg-slate-100/90 dark:bg-slate-800/90',
        r'(?<![\w:-])bg-slate-800/80(?![\w/-])': 'bg-slate-100/90 dark:bg-slate-800/80',
        r'(?<![\w:-])bg-slate-800/50(?![\w/-])': 'bg-slate-100/50 dark:bg-slate-800/50',
        
        r'(?<![\w:-])bg-slate-700(?![\w/-])': 'bg-slate-300 dark:bg-slate-700',
        r'(?<![\w:-])bg-slate-700/80(?![\w/-])': 'bg-slate-300/80 dark:bg-slate-700/80',
        r'(?<![\w:-])bg-slate-700/50(?![\w/-])': 'bg-slate-300/50 dark:bg-slate-700/50',
        
        r'(?<![\w:-])bg-black/80(?![\w/-])': 'bg-slate-300/90 dark:bg-black/80',
        r'(?<![\w:-])bg-black/50(?![\w/-])': 'bg-slate-300/70 dark:bg-black/50',

        # Text (softer, slate-800 instead of slate-900)
        r'(?<![\w:-])text-white(?![\w/-])': 'text-slate-800 dark:text-white',
        r'(?<![\w:-])text-slate-100(?![\w/-])': 'text-slate-800 dark:text-slate-100',
        r'(?<![\w:-])text-slate-200(?![\w/-])': 'text-slate-700 dark:text-slate-200',
        r'(?<![\w:-])text-slate-300(?![\w/-])': 'text-slate-600 dark:text-slate-300',
        r'(?<![\w:-])text-slate-400(?![\w/-])': 'text-slate-500 dark:text-slate-400',
        r'(?<![\w:-])text-slate-500(?![\w/-])': 'text-slate-500 dark:text-slate-500',
        
        # Borders (slightly darker for better separation on slate-50/100)
        ...
        # Accent Text (slightly softer, maybe keep 600 as it reads well)
...
print("Done making light mode softer.")
